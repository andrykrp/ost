...
WHERE (task0_.id IN (SELECT t.id FROM Task t INNER JOIN task_tag tt ON t.id=tt.task_id INNER JOIN Tag tg ON tt.tag_id=tg.id WHERE tg.title IN (<#list tags as tag>'${tag}'<#if tag_has_next>, </#if></#list>) GROUP BY t.id HAVING count(t.id)=${tags?size}))
